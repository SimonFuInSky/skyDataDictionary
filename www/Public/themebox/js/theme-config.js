...
var themeConfig = {
    ...
    initialize: function () {
        ...
        $('head').append($('<link rel="stylesheet">').attr('href', '/Public/themebox/js/theme-config.css'));
        $this.build();
        $this.events();
    ...
    events: function () {
        ...
        $('span.search-button').click(function (e) {
            e.preventDefault();
            if ($this.container.hasClass('active')) {
                $this.container.animate({
                    right: '-' + $this.container.width() + 'px'
    ...
    setColor: function (color) {
        ...
        $colorConfigLink.attr('href', '/Public/themebox/css/theme-' + color + '.css');
        $.cookie('color', color);
    },
